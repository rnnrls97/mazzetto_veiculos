...
exports.deprecations = exports.getDeprecationIds = exports.Version = void 0;
class Version {
 ...
 */
function isActiveIn(deprecation, version) {
    const deprecatedIn = deprecation.deprecatedIn;
    if (deprecation.status !== 'active' || !deprecatedIn)
        return false;
    if (version.major > deprecatedIn.major)
        return true;
    if (version.major < deprecatedIn.major)
        return false;
    if (version.minor > deprecatedIn.minor)
        return true;
    if (version.minor < deprecatedIn.minor)
        return false;
...
function getDeprecationIds(arr) {
    return arr.flatMap(item => {
        if (item instanceof Version) {
            return Object.values(exports.deprecations)
