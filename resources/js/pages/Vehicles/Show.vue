                                ...
                                <div class="column is-6 ">
                                    <VField>
                                        ...
                                        <VControl>
                                            ...
                                            />
                                            <VError :error="props.errors?.year_model" />
                                        </VControl>
                                    </VField>
                                </div>
                                    ...
                                    <VField>
                                        ...
                                        <VControl>
                                            <VTextarea
                                                ...
                                                :error="props.errors?.observation"
                                            />
                                        </VControl>
                                    </VField>
                                </div>
            ...
            <div class="form-outer">
                <div class="form-header stuck-header">
                    <div class="form-header-inner">
                        ...
                        <div class="right">
                            <div class="buttons">
                                <VButton color="primary" @click="saveImages" raised>
                                    Salvar
                                </VButton>
                            </div>
                        </div>
...
import { computed, onMounted, ref } from "vue";
    ...
    const vehicleImages = computed(() => filePondImages.value?.pond.getFiles());
    ...
    const getModelVersions = () => {
        ...
        axios.get(`/brand-models/${form.brand_model_id}/versions`)
            .then((response) => {
                ...
                console.log(form.model_version_id)
            });
    }
    ...
    const onSubmit = () => {
        form.put(route("vehicles.update", { vehicle: props.vehicle.id }), {
            preserveScroll: true,
            onSuccess: () => {
                console.log(notyf)
                notyf?.primary("Veículo atualizado com sucesso!");
            },
            onError: (error) => {
                console.log(error)
                notyf?.error("Erro ao atualizar veículo!");
            },
        });
    };

    const saveImages = () => {
        const images = filePondImages.value.pond.getFiles()
        formImage.images = images.map((image) => image.file);

        formImage.post(route("vehicles.store-images", { vehicle: props.vehicle.id }),{
                preserveScroll: true,
                onSuccess: () => {
                    formImage.reset();
                    notyf?.primary("Imagens atualizadas com sucesso!");
                },
            });
    };

    const setVehicleData = () => {
        brandId.value = props.vehicle.brand_model.brand_id;

        Object.keys(props.vehicle).forEach((key) => {
            if (form[key] !== undefined) {
                form[key] = props.vehicle[key];
            }

            if(key === 'accessories') {
                form[key] = props.vehicle[key].map((accessory) => accessory.id);
            }
        });
        
        if(props.vehicle.images.length > 0) {
            props.vehicle.images.forEach((image) => {
                ...
                filePondImages.value.pond.addFile(file.source, file.options);
    ...
    onMounted(() => setVehicleData());

    watch(() => brandId.value, () => {
        if(init.value){
            if(!props.vehicle.model_version_id){
                init.value = false
            }
